...
5. If the GitHub repository variable `FORTUNE_PUBLIC_SITE_URL` exists, update it to `https://fortunepad.fun`; otherwise the monitor's new default applies. Run the Public Site Monitor and retain its artifact. For full read-path verification, also set `FORTUNE_REQUIRE_READY=true FORTUNE_REQUIRE_ONCHAIN=true` when running the smoke command. Confirm the official X link remains `https://x.com/fortunepad`.
